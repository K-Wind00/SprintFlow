...
import { editCard, archiveCard } from '../../services/board'
...
import useStyles from '../../common/modalStyles'
		...
		<Modal open={open} onClose={() => setOpen(false)}>
			<div className={`${classes.paper} ${classes.cardModal}`}>
				...
				<div className={classes.modalSection}>
					...
					<div>
						<h3 className={classes.labelTitle}>Etykieta</h3>
						<GithubPicker className={classes.colorPicker} onChange={async color => dispatch(editCard(cardId, { label: color.hex }))} />
						<Button className={classes.noLabel} variant='outlined' onClick={async () => dispatch(editCard(cardId, { label: 'none' }))}>
							Bez etykiety
						</Button>
					</div>
				</div>
