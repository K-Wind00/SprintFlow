...
import { moveCard } from '../../services/board'
...
const MoveCard = ({ cardId, setOpen, thisList }) => {
	...
	return (
		<div className={classes.moveCard}>
			<h3 className={classes.moveCardTitle}>Przenieś to zadanie</h3>
			<div>
				<FormControl className={classes.moveCardSelect}>
					<InputLabel shrink>Lista</InputLabel>
					<Select value={listTitle} required onChange={e => { setListTitle(e.target.value)
							setListObject(
				...
				<FormControl className={classes.moveCardSelect}>
					<InputLabel shrink>Pozycja</InputLabel>
					<Select
						value={position}
						required
